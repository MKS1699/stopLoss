...
const PasswordField = ({
  ...
  return (
    ...
    >
      <h3 className="font-medium text-xl md:text-lg text-light self-end">
        Password
      </h3>
      <input
        ...
        id={passwordId}
        value={password}
        className="text-xl md:text-lg px-2 outline-none text-dark bg-white dark:bg-[#003b31] dark:text-light border-2 border-solid border-[#4CB050] rounded-md "
        onChange={(e) => setPassword(e.target.value)}
      />
      {showPassword ? (
        <IoEyeOff
          className="absolute w-8 h-8 bottom-3  md:bottom-2 right-4 text-bodyDark dark:text-light cursor-pointer"
          onClick={() => setShowPassword(false)}
        />
      ) : (
        <IoEye
          className={`absolute w-8 h-8 right-4 text-bodyDark dark:text-light cursor-pointer ${
